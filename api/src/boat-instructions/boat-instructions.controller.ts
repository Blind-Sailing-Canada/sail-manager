  ...
  async updateBoatInstructions(@Param('boat_id') boat_id: string, @Body() instructions) {
    ...
    await this.service.repository.manager.transaction(async transactionalEntityManager => {
      await Promise.all(boat.instructions
        ...
        .map((instruction) => transactionalEntityManager
          .update(BoatInstructionsEntity, { id: instruction.id }, instructions[instruction.id])));
    });

    return BoatEntity.findOne({ where: { id: boat_id } });
  }
}

...
export class EditUserAccessGuard implements CanActivate {
  constructor(
  ...
  async canActivate(): Promise<boolean> {
    const can: boolean = await this.canEditUserAccess();
  ...
  private async canEditUserAccess(): Promise<boolean> {
    ...
    const hasEditUserAccess = access[UserAccessFields.EditUserAccess];

    return isAdmin || hasEditUserAccess;
